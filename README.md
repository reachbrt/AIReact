<div align="center">
  <h1>🚀 ReactAI - Modern AI Components for React</h1>

  <p>
    <strong>13 Production-Ready AI Components | Multi-Provider Support | React 18+ Compatible</strong>
  </p>

  <!-- Primary Badges -->
  <p>
    <a href="https://www.npmjs.com/package/@aireact/core"><img src="https://img.shields.io/npm/v/@aireact/core.svg?style=for-the-badge&logo=npm&logoColor=white&color=CB3837" alt="npm version"></a>
    <a href="https://github.com/reachbrt/reactai/blob/main/LICENSE"><img src="https://img.shields.io/npm/l/@aireact/core.svg?style=for-the-badge&color=green" alt="MIT License"></a>
    <a href="https://github.com/reachbrt/reactai/stargazers"><img src="https://img.shields.io/github/stars/reachbrt/reactai?style=for-the-badge&logo=github&color=yellow" alt="GitHub Stars"></a>
    <a href="https://github.com/reachbrt/reactai/network/members"><img src="https://img.shields.io/github/forks/reachbrt/reactai?style=for-the-badge&logo=github&color=blue" alt="GitHub Forks"></a>
  </p>

  <!-- Technology Badges -->
  <p>
    <img src="https://img.shields.io/badge/React-18+-61DAFB?style=flat-square&logo=react&logoColor=white" alt="React">
    <img src="https://img.shields.io/badge/TypeScript-Ready-3178C6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript">
    <img src="https://img.shields.io/badge/OpenAI-Supported-412991?style=flat-square&logo=openai&logoColor=white" alt="OpenAI">
    <img src="https://img.shields.io/badge/Claude-Supported-CC9B7A?style=flat-square&logo=anthropic&logoColor=white" alt="Claude">
    <img src="https://img.shields.io/badge/Gemini-Supported-4285F4?style=flat-square&logo=google&logoColor=white" alt="Gemini">
    <img src="https://img.shields.io/badge/Ollama-Supported-000000?style=flat-square&logo=ollama&logoColor=white" alt="Ollama">
  </p>

  <!-- Quick Links -->
  <p>
    <a href="#-quick-start"><img src="https://img.shields.io/badge/⚡_Quick_Start-Begin-95E1D3?style=for-the-badge" alt="Quick Start"></a>
    <a href="#-component-packages"><img src="https://img.shields.io/badge/📦_Packages-View-4ECDC4?style=for-the-badge" alt="Packages"></a>
    <a href="https://github.com/reachbrt/reactai/issues/new"><img src="https://img.shields.io/badge/🐛_Report_Bug-Submit-F7B731?style=for-the-badge" alt="Report Bug"></a>
  </p>

  ---

  <h3>⭐ If you find this project useful, please consider giving it a star! ⭐</h3>

  <p>
    <em>Built with ❤️ by <a href="https://github.com/reachbrt">reachbrt</a></em>
  </p>
</div>

## ✨ Features

A modular AI-powered React component suite that enhances your applications with intelligent capabilities:

<table>
  <tr>
    <td align="center" width="16.66%">
      <b>@aireact/chatbot</b><br>
      <small>Multi-provider AI chat widget</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/autosuggest</b><br>
      <small>Smart autosuggest with semantic ranking</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/smartform</b><br>
      <small>AI-enhanced forms with dynamic validation</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/image-caption</b><br>
      <small>AI-powered image captioning</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/emotion-ui</b><br>
      <small>Emotion-aware UI components</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/doc-intelligence</b><br>
      <small>Document processing with OCR</small>
    </td>
  </tr>
  <tr>
    <td align="center" width="16.66%">
      <b>@aireact/predictive-input</b><br>
      <small>AI-powered text predictions</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/smart-notify</b><br>
      <small>Intelligent notifications</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/voice-actions</b><br>
      <small>Voice command integration</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/analytics</b><br>
      <small>AI-powered analytics</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/smart-datatable</b><br>
      <small>AI-native data table</small>
    </td>
    <td align="center" width="16.66%">
      <b>@aireact/360-spin</b><br>
      <small>360° product viewer with AI generation</small>
    </td>
  </tr>
</table>

---

## 🌟 Why Choose @aireact?

<table>
<tr>
<td width="33%" align="center">

### 🎯 **Production Ready**
Battle-tested components with full TypeScript support, comprehensive types, and detailed documentation.

</td>
<td width="33%" align="center">

### 🔌 **Multi-Provider**
Works with OpenAI, Claude, Gemini, Ollama, DeepSeek, and HuggingFace. Switch providers with one line of code.

</td>
<td width="33%" align="center">

### ⚡ **Easy Integration**
Install via npm, add your API key, and start building. Works with React 18+, Next.js, and all modern build tools.

</td>
</tr>
<tr>
<td width="33%" align="center">

### 🎨 **Customizable**
Full control over styling, behavior, and AI parameters. Built with flexibility in mind.

</td>
<td width="33%" align="center">

### 📦 **Modular**
Install only what you need. Each component is a separate package with zero dependencies on others.

</td>
<td width="33%" align="center">

### 🚀 **Modern Stack**
Built with React 18, TypeScript, Vite, and modern hooks. Optimized for performance.

</td>
</tr>
</table>

---

## 📊 Project Stats

<div align="center">

| Metric | Value |
|--------|-------|
| 📦 **Total Packages** | 13 packages |
| ⭐ **GitHub Stars** | ![GitHub Stars](https://img.shields.io/github/stars/reachbrt/reactai?style=social) |
| 🍴 **Forks** | ![GitHub Forks](https://img.shields.io/github/forks/reachbrt/reactai?style=social) |
| 🐛 **Open Issues** | ![GitHub Issues](https://img.shields.io/github/issues/reachbrt/reactai) |
| 📝 **License** | MIT |

</div>

---

## 🚀 Quick Start

### 📦 Installation

```bash
# Install the core package
npm install @aireact/core

# Install component packages as needed
npm install @aireact/chatbot
npm install @aireact/autosuggest
npm install @aireact/smartform
npm install @aireact/image-caption
npm install @aireact/emotion-ui
npm install @aireact/doc-intelligence
npm install @aireact/predictive-input
npm install @aireact/smart-notify
npm install @aireact/voice-actions
npm install @aireact/analytics
npm install @aireact/smart-datatable
npm install @aireact/360-spin
```

### 🔑 API Key Setup

Create a `.env` file in your project root:

```bash
# OpenAI API Key
VITE_OPENAI_API_KEY=your_openai_api_key_here

# Claude API Key (optional)
VITE_CLAUDE_API_KEY=your_claude_api_key_here

# Gemini API Key (optional)
VITE_GEMINI_API_KEY=your_gemini_api_key_here
```

> **Note:** Components include fallback providers that work without API keys during development.

---

## 🏗️ Project Structure

```
reactai/
├── packages/
│   ├── core/              # @aireact/core - Foundation package
│   ├── chatbot/           # @aireact/chatbot - AI chat components
│   ├── autosuggest/       # @aireact/autosuggest - Smart suggestions
│   ├── smartform/         # @aireact/smartform - AI form validation
│   ├── image-caption/     # @aireact/image-caption - Image captioning
│   ├── emotion-ui/        # @aireact/emotion-ui - Emotion-aware UI
│   ├── doc-intelligence/  # @aireact/doc-intelligence - Document OCR
│   ├── predictive-input/  # @aireact/predictive-input - Text predictions
│   ├── smart-notify/      # @aireact/smart-notify - Smart notifications
│   ├── voice-actions/     # @aireact/voice-actions - Voice commands
│   ├── analytics/         # @aireact/analytics - AI analytics
│   ├── smart-datatable/   # @aireact/smart-datatable - AI data tables
│   └── 360-spin/          # @aireact/360-spin - 360° product viewer
├── demo/                  # Live demo application
└── package.json           # Root workspace configuration
```

---

## 📦 Component Packages

### @aireact/core

**The Foundation Package** - All other packages depend on this.

```tsx
import { AIClient } from '@aireact/core';

const client = new AIClient({
  provider: 'openai',
  apiKey: import.meta.env.VITE_OPENAI_API_KEY,
  model: 'gpt-4o'
});

// Chat
const response = await client.chat([
  { role: 'user', content: 'Hello!' }
]);

// Streaming
client.chatStream(
  [{ role: 'user', content: 'Write a poem' }],
  {
    onToken: (token) => console.log(token),
    onComplete: (text) => console.log('Done:', text)
  }
);
```

**Supported Providers:**
- OpenAI (GPT-4, GPT-4o, GPT-3.5-turbo)
- Anthropic (Claude 3.5 Sonnet, Claude 3 Opus)
- Google (Gemini Pro, Gemini Ultra)
- HuggingFace (Open-source models)
- Ollama (Local LLM deployment)
- DeepSeek (DeepSeek models)
- Fallback (Mock responses for development)

---

### @aireact/chatbot

**Enterprise-Grade Conversational AI**

```tsx
import { ChatWindow } from '@aireact/chatbot';
import '@aireact/chatbot/dist/chatbot.css';

function App() {
  return (
    <ChatWindow
      provider="openai"
      apiKey={import.meta.env.VITE_OPENAI_API_KEY}
      model="gpt-4o"
      placeholder="Ask me anything..."
      showAvatars={true}
      theme="light"
      streaming={true}
    />
  );
}
```

**Features:**
- 💬 Multi-provider AI chat
- 🎤 Voice input support
- 📎 File attachments
- 🔄 Streaming responses
- 🎨 Customizable themes

---

### @aireact/360-spin

**Interactive 360° Product Viewer with AI Generation**

```tsx
import { SpinGenerator } from '@aireact/360-spin';
import '@aireact/360-spin/dist/360-spin.css';

function App() {
  return (
    <SpinGenerator
      apiKey={import.meta.env.VITE_OPENAI_API_KEY}
      numFrames={8}
      quality="hd"
      width="100%"
      height={500}
    />
  );
}
```

**Features:**
- 🔄 360° drag-to-rotate viewer (like Ortery)
- 🤖 AI-powered view generation from single image
- 📸 Dynamic angled product shots
- 📱 Touch/swipe support for mobile
- 🎨 Premium e-commerce photography style

---

### @aireact/autosuggest

**AI-Powered Smart Suggestions**

```tsx
import { Autosuggest } from '@aireact/autosuggest';

function App() {
  const [query, setQuery] = useState('');

  return (
    <Autosuggest
      value={query}
      onChange={setQuery}
      apiKey={import.meta.env.VITE_OPENAI_API_KEY}
      placeholder="Search..."
      debounce={300}
      onSelect={(suggestion) => console.log(suggestion)}
    />
  );
}
```

---

### @aireact/smartform

**AI Form Validation & Auto-Correction**

```tsx
import { SmartForm } from '@aireact/smartform';

const schema = {
  email: { type: 'email', required: true, aiValidation: true },
  phone: { type: 'tel', required: true, aiValidation: true },
  address: { type: 'text', aiValidation: true }
};

function App() {
  return (
    <SmartForm
      schema={schema}
      apiKey={import.meta.env.VITE_OPENAI_API_KEY}
      onSubmit={(data) => console.log(data)}
    />
  );
}
```

---

### @aireact/image-caption

**AI Image Captioning with GPT-4 Vision**

```tsx
import { ImageCaption } from '@aireact/image-caption';

function App() {
  return (
    <ImageCaption
      apiKey={import.meta.env.VITE_OPENAI_API_KEY}
      model="gpt-4o"
      onCaptionGenerated={(result) => console.log(result.caption)}
    />
  );
}
```

---

### @aireact/analytics

**AI-Powered Analytics & Insights**

```tsx
import { AnalyticsDashboard, useAnalytics } from '@aireact/analytics';

function App() {
  const analytics = useAnalytics({
    enabled: true,
    trackInteractions: true
  });

  return (
    <AnalyticsDashboard
      apiKey={import.meta.env.VITE_OPENAI_API_KEY}
      showConversationAnalytics={true}
    />
  );
}
```

---

### @aireact/smart-datatable

**AI-Native Data Tables with NLP Queries**

```tsx
import { SmartDataTable } from '@aireact/smart-datatable';

const data = [
  { id: 1, name: 'Product A', price: 99.99, category: 'Electronics' },
  { id: 2, name: 'Product B', price: 149.99, category: 'Clothing' }
];

const columns = [
  { key: 'id', label: 'ID', sortable: true },
  { key: 'name', label: 'Name', sortable: true },
  { key: 'price', label: 'Price', sortable: true },
  { key: 'category', label: 'Category', sortable: true }
];

function App() {
  return (
    <SmartDataTable
      data={data}
      columns={columns}
      apiKey={import.meta.env.VITE_OPENAI_API_KEY}
      aiSearch={true}
      aiInsights={true}
    />
  );
}
```

**Features:**
- 🗣️ Natural language queries
- 🧠 Auto-insights with AI
- 🤖 Row-level AI agents
- ✨ AI transformations

---

## 🧪 Development

```bash
# Clone the repository
git clone https://github.com/reachbrt/reactai.git
cd reactai

# Install dependencies
npm install

# Start the demo
npm run dev --workspace demo

# Build all packages
npm run build --workspaces

# Build specific package
npm run build --workspace @aireact/core
```

---

## 🤝 Contributing

Contributions are welcome! Please read our contributing guide before submitting a pull request.

1. Fork the repository
2. Create your feature branch (`git checkout -b feature/amazing-feature`)
3. Commit your changes (`git commit -m 'Add amazing feature'`)
4. Push to the branch (`git push origin feature/amazing-feature`)
5. Open a Pull Request

---

## 🔗 Related Projects

| Framework | Repository | Status |
|-----------|-----------|--------|
| **Vue.js** | [vueai](https://github.com/reachbrt/vueai) | ✅ Published |
| **Angular** | [angularai](https://github.com/reachbrt/angularai) | 🚧 Coming Soon |
| **Svelte** | [svelteai](https://github.com/reachbrt/svelteai) | 💡 Planned |

---

## 📄 License

**MIT License** © 2025 [Bharatkumar Subramanian](https://github.com/reachbrt)

---

<div align="center">

### 🎉 **Thank you for using @aireact!**

<p>
  <strong>If this project helped you, please consider:</strong><br>
  ⭐ Starring the repository<br>
  🐦 Sharing on social media<br>
  🤝 Contributing to the codebase
</p>

<p>
  <em>Made with ❤️ by <a href="https://github.com/reachbrt">reachbrt</a></em>
</p>

---

**[⬆ Back to Top](#-reactai---modern-ai-components-for-react)**

</div>

